...
myhost=`hostname`
echo $manip $myhost >> /TopStor/grafana.files/hosts
echo $manip $myhost >> /TopStor/grafana.files/hosts
...
then
 ...
 docker run -d --name=prom --restart=always -p 9090:9090 -v /TopStor/prometheus.files/prometheus.yml:/etc/prometheus/prometheus.yml 10.11.11.124:5000/prom  
 ...
 docker run -d --name=grafana --restart=always -p 3000:3000 -v /TopStor/grafana.files/hosts:/etc/hosts -v "/TopStor/grafana:/etc/grafana" 10.11.11.124:5000/grafana
fi
